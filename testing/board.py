...
from testing.boardEvalutation import eval,compareDistance,possiblesquares

from players.strategy.theHeuristic import *

labels=[]

def draw_board(game,board,action=None,result=None):
    global oldBoard, lastMove, labels
    ...
    for (r,c) in board:
        if board[(r,c)] == 'K':
            ...
            myKing=(r,c)
            myGoal=(0,7)
    symbols={'o':1.0,'Q':1.0,'x':-1.0,'K':-1.0,'#':0,' ':0}

    #print(compareDistance(myKing,myGoal,opponentKing,opponentGoal))

    (r,c)=myKing
    for i in range(1,3):
        point=(r-i,c-i)
        value=float("{:.2f}".format(eval(board,'O',symbols,myKing,opponentKing,myGoal,opponentGoal,point)))
        label = ax.annotate(
            value,
            xy=(point[1] + 0.5, board.height - 1 - point[0] + 0.5),
            fontsize=10,
